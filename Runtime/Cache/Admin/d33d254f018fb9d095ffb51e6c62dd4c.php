...
<script type="text/javascript">
  $.post("<?php echo U('Admin/Trade/getxiaoxi');?>",
        // {'st':st},
        function(data){
            if(data.code == 1){
				...
				<tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
								    ...
								    <span>认购数量：<?php echo ($vo["onum"]); ?></span>
								</td>
								<td>
								...
								<td>
								    <span>当前收益：<?php echo ($vo["lixi_money"]); ?> ( <?php echo ($vo["lixi"]); ?>)%</span><br />
								    <span>预估总收益：<?php echo ($vo["yugu"]); ?> (<?php echo ($vo["lockday"]); ?>天)</span>
								</td>
								<td>
