			...
			<table class="">
				...
				<tbody>
                    <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($vo["buy_orblance"]); ?></td>
						<td>
						    	 <input type="button" class="ajax-get btn btn-<?php if($vo['kongyk'] == 0){echo('danger');}else{echo('warning');} ?> btn-xs" value="正常<?php if($vo['kongyk'] == 0){echo('(当前)');} ?>" onclick="buy_zhengchang('<?php echo ($vo['id']); ?>');"> 
			 <input type="button" class="ajax-get btn btn-<?php if($vo['kongyk'] == 1){echo('danger');}else{echo('warning');} ?> btn-xs" value="盈利<?php if($vo['kongyk'] == 1){echo('(当前)');} ?>" onclick="buy_ying('<?php echo ($vo['id']); ?>');">
			 <input type="button" class="ajax-get btn btn-<?php if($vo['kongyk'] == 2){echo('danger');}else{echo('warning');} ?> btn-xs" value="亏损<?php if($vo['kongyk'] == 2){echo('(当前)');} ?>" onclick="buy_kui('<?php echo ($vo['id']); ?>');">
